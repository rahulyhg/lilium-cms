...
var ArticleHelper = function() {
    ...
    this.getNewest = function(cli, extra, cb) {
        db.aggregate(cli._c, 'content', [{$match: {_id : {$ne : db.mongoID(extra._id)}, status : 'published'}}, {$sort : {'date' : -1}}, {$limit : 3}, {$project: {name: 1, title: 1, category:1, media:1,  categories: 1}}, {$lookup : {from: 'uploads', localField: 'media', foreignField: '_id', as : 'media'}},{$unwind : {path: "$categories",preserveNullAndEmptyArrays: true}}, {$lookup : {from: 'categories', localField: 'categories', foreignField: 'name', as : 'category'}},{$unwind : {path: "$category",preserveNullAndEmptyArrays: true}}, {$unwind : {path: "$media",preserveNullAndEmptyArrays: true}}], function(res) {
            extra.newest = res;
            if (extra.newest[0]) {
    ...
    this.homepageTrending = function (cli, extra, cb) {
        db.aggregate(cli._c, 'content', [{$match : {'trending' : true}}, {$limit : 3}], function(res) {
            extra.trending = res;
            if (extra.trending[0]) {
    ...
    this.homepageNew = function (cli, extra, cb) {
        db.aggregate(cli._c, 'content', [{$sort : {date : -1}}, {$limit : 8}], function(res) {
            extra.new = res;
            if (extra.new[0]) {
    ...
    this.homepageBestOf = function(cli, extra, cb) {
        db.aggregate(cli._c, 'content', [{$match: {categories : {$in: [{$regex: "/(best-of.*)/"}]}}}, {$sample : {size: 3}}], function(res) {
            extra.bestof = res;
            if (extra.bestof[0]) {
    ...
    this.homepageThingsToDo = function(cli, extra, cb) {
        db.aggregate(cli._c, 'content', [{$match: {categories : { $in: [{$regex: "/(things-to-do.*)/"}]}}}, {$sample : {size: 3}}], function(res) {
