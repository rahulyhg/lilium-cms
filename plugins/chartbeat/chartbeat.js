...
var Chartbeat = function() {
	...
	var registerSettings = function() {
		hooks.bind('settings_form_bottom', 400, function(pkg) {
			...
			.add('chartbeat.api_key', 'text', {displayname:"API Key"}, {required:false})
			.add('chartbeat.configuid', 'text', {displayname:"Configuration ID"}, {required:false})
			.add('chartbeat.host', 'text', {displayname:"Host"}, {required:false})
			.add('chartbeat.section', 'text', {displayname:"Section (blank if none)"}, {required:false});
		});
					...
					switch (levels[1]) {
						...
						default :
							...
							db.find(cli._c, 'chartbeathisto', {}, {}, function(err, cur) {
								if (!err) {
									cur.sort({"_id" : -1}).limit(limit).toArray(function(err, dat) {
										send(dat);
									});
								} else {
									send(err);
