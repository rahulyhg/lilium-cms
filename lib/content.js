            ...
            db.findUnique(config.default(), 'entities', { _id : caller }, (err, fullrequester) => {
                try {
                    if (fullrequester.roles.includes('editor') || !post.author || caller.toString() == post.author.toString()) {
                        ...
                        let error = {};
                        if (!post.title || !post.title[0]) error.title = { message: 'Must include a title' };
                        if (!post.subtitle || !post.subtitle[0]) error.subtitle = { message: 'Must include a subtitle' };
                        if (!post.editions || post.editions.length == 0) error.editions = { message: 'Must have at least one edition' };
                        if (!post.content || !post.content[0]) error.content = { message: 'Must include some content' };
                        if (!post.media) error.media = { message: 'Must include a featured media' };

                        error.forEach(e => e.setBy = 'lilium');

                        hooks.fire('article_validating', error);

                        if (Object.keys(error).length) return sendback({ error, code : 400 });
