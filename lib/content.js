...
const PUBLICATION_REPORT_TODAY_PROJECTION = {
    ...
    date : 1, name : 1, facebookmedia : 1, author : 1, url : 1
};
...
class ContentLib {
    ...
    generate(_c, deepArticle, cb, pageIndex) {
        ...
        const nextHook = () => {
            if (++hookIndex != asyncHooks.length) {
                ...
                setImmediate(() => require('./amp').clearAmpCache(_c, deepArticle));

                // Generate LML page
                if (extra.cancelRender) {
                    cb && cb({
                        success : true,
                        cancelled : true, 
                        deepArticle
    ...
    updateActionStats(_c, deepArticle, callback, reduce) {
        ...
        db.update(require('../lib/config').default(), 'actionstats', {entity, type : "article_content"}, stats, (err, r)  => {
            ...
            db.count(_c, 'content', { author : article.author }, (err, authortotal) => {
                db.count(_c, 'content', { author : article.author, date }, (err, authortotaltoday) => {
                    db.count(_c, 'content', { status : "published" }, (err, websitetotal) => {
                        db.count(require('../lib/config').default(), 'decorations', { entity : article.author }, (err, decorations) => {
                            db.join(_c, 'content', [
                                ...
                                lastpublished.push({
                                    ...
                                    author : article.author._id || article.author, editions : article.editions, url : article.url
                                });
                    ...
                    this.pushHistoryEntryFromDiff(_c, postid, caller, deepdiff, 'update', entry => {
                        if (entry) {
                            this.fetchDeepFieldsFromDiff(_c, newpost, entry, () => {
                                ...
                                hooks.fireSite(_c, 'article_will_update', { article : oldpost, updated : newpost, full : false });
            ...
            db.findUnique(config.default(), 'entities', { _id : caller }, (err, fullrequester) => {
                try {
                    if (fullrequester.roles.includes('editor') || !post.author || caller.toString() == post.author.toString()) {
                        ...
                        // Reference might be modified by a validation plugin
                        // should be structured as { <fieldName>: { message: <error message>, setBy: <pluginName> } }
                        // setBy is an optional field that a validation plugin might set to indicate what plugin generated what validation error
                        let errors = {};
                        if (!post.title || !post.title[0]) errors.title = { message: 'Must include a title' };
                        if (!post.subtitle || !post.subtitle[0]) errors.subtitle = { message: 'Must include a subtitle' };
                        if (!post.editions || post.editions.length == 0) errors.editions = { message: 'Must have at least one edition' };
                        if (!post.content || !post.content[0]) errors.content = { message: 'Must include some content' };
                        if (!post.media) errors.media = { message: 'Must include a featured media' };

                        for (let key in errors) {
                            errors[key].setBy = 'lilium';
                        }

                        hooks.trigger('article_validating', { errors, post });

                        if (Object.keys(errors).length) return sendback({ errors, code : 400 });
                        else return sendback(undefined, post);
                    } else {
                        return sendback({ error: { right: 'Missing rights' }, code : 403 });
                    }
                } catch (err) {
    ...
    calculateArticleWorth(post) {
        ...
        else if (post.wordcount >= 600) return 35;
        else return 25;
    }
                ...
                this.updateActionStats(_c, fullpost, () => {
                    ...
                    if (fullpost.fulltopic && fullpost.title && fullpost.title.length > 1) {
                        fullpost.title.forEach((x, i) => {
                            fs.unlink(_c.server.html + "/" + fullpost.fulltopic.completeSlug + "/" + fullpost.name + "/" + (i+1) + ".html", () => {});
                        })
                    } else if (fullpost.fulltopic) {
                        fs.unlink(_c.server.html + "/" + fullpost.fulltopic.completeSlug + "/" + fullpost.name + ".html", () => {});
                    }

                    fs.unlink(_c.server.html + "/amp/" + fullpost.name + ".html", () => {});
                }, true);
            });
        });
    }
    ...
    sendPreviewViaEmail(_c, postid, emails, sender, sendback) {
        db.findUnique(_c, 'content', { _id : postid }, (err, article) => {
            if (!article) {
                sendback(404);
            } else {
                emails.forEach(email => {
                    db.insert(_c, 'guestemails', {
                        email,
                        postid
                    }, (err, r) => {
                        require('./mail.js').triggerHook(_c, 'preview_sent_via_email', email, {
                            to : email, 
                            guestemailid : r.insertedId,
                            previewlink : _c.server.protocol + _c.server.url + "/preview/" + article._id + "/" + article.previewkey + "/" + r.insertedId,
                            article
                        })
                    });
                });
