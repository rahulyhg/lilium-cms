...
module.exports = class Money {
    ...
    dispatchOne(contractorid, currency, requester, done) {
        const mainsite = require('../config').default();
                                    ...
                                    const handleSite = () => {
                                        ...
                                        db.update(siteid, 'content', { _id : { $in : dbposts[siteid].map(x => x._id) } }, {
                                            ...
                                            if (r && r.modifiedCount != 0 && invoice.valid) {
                                                db.findUnique(require('../config').default(), 'entities', { _id : contractorid }, (err, contractor) => {
                                                    if (contractor && contractor.email) {
                                                        require("../lml3/compiler").compile(
                                                            require('../config').fetchConfig(siteid), 
                                                            liliumroot + "/backend/dynamic/invoice.lml3",
                                                            { invoice, contractor },
                                                            markup => {
                                                                mailLib.triggerHook(require('../config').fetchConfig(siteid), 'send_invoice_to_contractor', contractor.email, {
                                                                    contractor,
                                                                    invoice,
                                                                    markup
                                                                }, () => {
