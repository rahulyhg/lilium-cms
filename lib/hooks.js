...
class Hooks {
    ...
    getAsyncHooksFor(_c, name) {
        return asyncsiteevents[_c.id] ? asyncsiteevents[_c.id][name] : [];
    }
    ...
    bindAsync(_c, name, cb) {
        asyncsiteevents[_c.id] = asyncsiteevents[_c.id] || {};
        asyncsiteevents[_c.id][name] = asyncsiteevents[_c.id][name] || [];
        asyncsiteevents[_c.id][name].push(cb);
    }
    ...
    fireAsync(_c, name, pkg, done) {
        let i = -1;
        const evts = (asyncsiteevents[_c.id] && asyncsiteevents[_c.id][name]) || [];

        const nextHook = () => {
            if (++i == evts.length) {
                done && done();
            } else {
                evts[i](_c, pkg, shouldContinue => {
                    shouldContinue === false ? done && done() : nextHook();
                });
            }
        };
