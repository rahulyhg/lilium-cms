...
var SiteInitializer = function (conf, siteobj) {
    ...
    var loadHTMLStructure = function (done) {
        ...
        fs.mkdir(conf.server.html, { recursive : true }, function (err, valid) {
            fs.mkdir(conf.server.html + "/next", { recursive : true }, function(err, nextvalid) {
                fs.mkdir(conf.server.html + "/amp", { recursive : true }, function(err, nextvalid) {
                    fs.mkdir(conf.server.html + "/api", { recursive : true }, function(err, nextvalid) {
                        log('Sites', 'HTML Directory was validated at : ' + conf.server.html, 'success');
