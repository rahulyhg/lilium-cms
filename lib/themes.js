...
var Themes = function () {
    ...
    this.getThemesDirList = function (conf, callback) {
        var themedir = require('path').join(liliumroot, 'flowers');
        fs.readdir(themedir, function (err, dirs) {
            ...
            nextDir = function () {
                ...
                if (i >= dirs.length) {
                    ...
                    var infoPath = require('path').join(themedir, dirs[i], "info.json");
    ...
    this.enableTheme = function (config, uName, callback, curSettings) {
        if (this.isActive(config, uName)) {
            ...
            this.searchDirForThemes(uName, function (info) {
                ...
                var themedir = require('path').join(liliumroot, 'flowers');
                var ThemeClass = require(require('path').join(themedir, info.dirName, info.entry));
