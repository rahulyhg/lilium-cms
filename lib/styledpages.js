...
class StyledPagesLib {
    ...
    generatePage(conf, page, done) {
        ...
        hooks.fireSite(conf, 'styledpage_will_render', { page });
        if (page.skiplayout) {
            fs.writeFile(pathlib.join(conf.server.html, landingPath), page.content, { encoding : 'utf8' }, () => done && done());
        } else {
            filelogic.renderThemeLML3(conf, 'styledpage', landingPath, extra, () => {
                done();
                hooks.fireSite(conf, 'styledpage_did_render', { page });
            });
        }
    }
    ...
    sendPage(cli, page) {
        ...
        let go = () => {
            fs.readFile(cli._c.server.html + "/" + landingPath, { encoding : 'utf8' }, (err, ctn) => {
                err ? cli.throwHTTP(404, undefined, true) : cli.sendHTML(ctn);
            });
        };

        fs.stat(landingPath, (err) => {
            if (err) {
