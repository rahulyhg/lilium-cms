...
class LiliumSocket { 
    ...
    disconnect() {
        ...
        var memObj = {};
        sharedcache.socket(uid, 'remove', sid, (remainingSessions) => {
            sharedcache.get("hit_" + sid, data => {
                data && db.insert(ls.config, 'hits', {
                    userid : db.mongoID(uid),
                    path : data.path,
    ...
    notificationInteraction(notifId) {
        ...
        // Save it
        sessionManager.saveSession(cli, () => {});
    };
...
class Notification {
    onSocketConnection(socket, uid) {
        ...
        if (sessionId) {
            ...
            sessionManager.getSessionFromSID(sessionId, (session) => {
                if (session) {
                    sharedcache.socket(session.data._id, 'add', socket.id, () => { 
    ...
    init() {
        if (global.liliumenv.mode != "script" && !global.liliumenv.caij && !process.env.job) {
            ...
            _c.eachSync((conf) => {
                ...
                ((uid) => { 
                    io.of(path + uid).on('connection', (socket) => {
                        this.onSocketConnection(socket, uid);
                    });
                })(conf.uid);
    ...
    notifyUser(userID, dbId, notification, difftype) {
        ...
        !difftype && db.insert(dbId, 'notifications', notification, () => {});

        // Send to every sockets connected by the user (for multi-tab)
        sharedcache.getSocketID(userID, (sockets) => {
    ...
    insertBatchNotificationInSessions(sessionsIDs, notification, site) {
        ...
        for (var sessionID in sessionsIDs) {
            ...
            if (session) {
                ...
                this.insertNotif(session, notification);
            }
        }
    ...
    emitToWebsite(siteid, data, type) {
        metrics.plus('socketevents');
        require('../pipeline/inbound.js').io().of(idToNamespace[siteid]).emit(type || 'notification', data);
    };

    messageNotif(user, msg, type) {
        metrics.plus('socketevents');
        sharedcache.getSocketID(user, (sockets) => {
            for (var i = 0; i < sockets.length; i++) {
                var split = sockets[i].split('#');
    ...
    broadcastNotification(notification) {
        ...
        var broadcast = (site) => {
            ...
            db.aggregate(site, 'entities', [{
                ...
                db.insert(site, 'notifications', result, (err, r) => {});
            });
            ...
            db.aggregate(site, 'sessions', [{
                ...
                this.insertBatchNotificationInSessions(result, notification, site);
            });
        };
    ...
    registerLiveVar() {
        livevars.registerLiveVariable('notifications', (cli, levels, params, cb) => {
        ...
        livevars.registerLiveVariable('online', (cli, levels, params, cb) => {
            if (levels[0] == "list") {
                sharedcache.getSocketID(false, (sockets) => {
                    cb(sockets);
                });
